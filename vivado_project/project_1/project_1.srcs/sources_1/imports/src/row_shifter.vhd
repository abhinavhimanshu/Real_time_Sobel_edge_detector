...
port(
	--input ports
		...
		data_in : in unsigned(DATA_WIDTH-1 downto 0); -- 8 bit data word recieved from UART usually except when edge detector is in state 11
		in_valid : in std_logic; -- valid input signal
		done_pulse : in std_logic; --indication that computation of last image is done, READY the Buffers for next image, this is used to time the flushing of ROW buffers
	--output ports	
		row_1 : out unsigned (DATA_WIDTH-1 downto 0); -- Output goes to Sobel filter
		row_2 : out unsigned(DATA_WIDTH-1 downto 0);  -- Output goes to Sobel filter
		row_3 : out unsigned(DATA_WIDTH-1 downto 0)    -- Output goes to Sobel filter
			...
			if(rst = '1') then
			   ...
			   if(done_pulse='1') then
			            if(in_valid = '1') then
						  ...
						  row_buffer1 <= (others=>(others => '0')); -- Flushing of ROW buffer
						  row_buffer2 <= (others=>(others => '0'));	-- Flushin of Row buffer							   				
			   else
					if(in_valid = '1') then -- Shiftin of data through ROW buffers
