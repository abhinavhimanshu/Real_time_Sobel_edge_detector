% author: Steve Medie
% filename : HW_SOBEL.m
% Description : This is Sobel filter without localisation of edges as it will work in our HW implementation
...
file_hnd=fopen('./outputref.txt','w');
for i =1:LEN+H+3
      ...
      %COMBINATIONAL PART STARTS
        ...
        fprintf(file_hnd,'%s\n',dec2bin(G,8));%currently we are checking our results against the results stored in this file, we will add mechanism later to remove initial garabage values later
...
%         fprintf(file_hnd,'%s\n',[dec2bin(out(ii,jj),8)]);
%        else
%         fprintf(file_hnd,'%s;',[dec2bin(I(ii,jj),8)]);
%        end
%     end
% end
